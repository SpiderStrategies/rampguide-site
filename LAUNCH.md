...
| 5 | **A contact address.** The site uses `hello@rampguide.com` (pricing FAQ, footer, welcome page, drafts). rampguide.com has **no MX records** (dropped at the DNS move — see rampguide-api/OPERATIONS.md), so today that address bounces. Add a forwarder (ImprovMX/SES) or change the address everywhere before go-live. | Nate | open |
...
| 7 | **Two pricing-page claims to eyeball** (see the handoff deliverable message): "Hosted GRC platforms typically run $25,000–55,000 a year" and "278 of the 323 Rev5 Moderate controls mapped" (derived from catalog.json; re-run the count when packs change). | Nate | review |
